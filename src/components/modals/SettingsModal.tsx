...
const DEFAULT_SUBTAB_BY_CATEGORY: Record<SettingsCategory, SettingsSubtab> = {
  app: 'appearance',
  tasks: 'defaults',
...
export const SettingsModal = ({
  ...
  const [activeSelection, setActiveSelection] = useState<{
    ...
    const category = initialCategory || 'app';
    return {
      category,
      subtab:
        initialCategory === category && initialSubtab
          ? initialSubtab
          : DEFAULT_SUBTAB_BY_CATEGORY[category],
    };
  });
  ...
  const { category: activeCategory, subtab: currentSubtab } = activeSelection;
    ...
    >
      <div className="flex h-[calc(85vh-12rem)] max-h-[75vh] overflow-hidden [@media(max-height:900px)]:h-[calc(100vh-10rem)] [@media(max-height:900px)]:max-h-[calc(100vh-10rem)]">
        <nav
          ...
          className="min-h-0 w-56 shrink-0 space-y-4 overflow-y-auto overscroll-contain rounded-l-xl border-surface-300 border-r bg-white p-3 dark:border-surface-700 dark:bg-surface-800"
        >
            ...
            <div key={category.id} className="space-y-2">
              ...
              <div className="space-y-1">
                {category.subtabs.map((tab) => {
                  const isActiveTab = activeCategory === category.id && currentSubtab === tab.id;
        ...
        <div
          ref={contentRef}
          className="settings-modal-content min-h-0 min-w-0 flex-1 overflow-y-auto overscroll-contain p-6"
        >
          ...
          {activeCategory === 'misc' && (
            <>
              ...
              {currentSubtab === 'about' && (
                <AboutSettings
                  onNavigateToUpdates={() => {
                    setActiveSelection({ category: 'misc', subtab: 'updates' });
                    resetContentScroll();
                  }}
                />
              )}
